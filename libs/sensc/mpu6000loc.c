...
int mpu6000_getData(mpu6000_ctx_t *ctx, sensor_event_t *gyroEvt, sensor_event_t *accelEvt)
{
	...
	accelEvt->accels.temp = translateTemp(ibuf[6], ibuf[7]);
	gyroEvt->gyro.temp =accelEvt->accels.temp;

	accelEvt->accels.accelX = translateAcc(ibuf[0], ibuf[1]);
	accelEvt->accels.accelY = translateAcc(ibuf[2], ibuf[3]);
	accelEvt->accels.accelZ = translateAcc(ibuf[4], ibuf[5]);
	accelEvt->timestamp = tStamp;

	gyroEvt->gyro.gyroX = translateGyr(ibuf[8], ibuf[9]);
	gyroEvt->gyro.gyroY = translateGyr(ibuf[10], ibuf[11]);
	gyroEvt->gyro.gyroZ = translateGyr(ibuf[12], ibuf[13]);
	gyroEvt->timestamp = tStamp;
	...
	dAngleX += (uint32_t)((gyroEvt->gyro.gyroX + lastGyroX) * step);
	dAngleY += (uint32_t)((gyroEvt->gyro.gyroY + lastGyroY) * step);
	dAngleZ += (uint32_t)((gyroEvt->gyro.gyroZ + lastGyroZ) * step);

	gyroEvt->gyro.dAngleX = dAngleX;
	gyroEvt->gyro.dAngleY = dAngleY;
	gyroEvt->gyro.dAngleZ = dAngleZ;

	tStampLast = tStamp;
	lastGyroX = gyroEvt->gyro.gyroX;
	lastGyroY = gyroEvt->gyro.gyroY;
	lastGyroZ = gyroEvt->gyro.gyroZ;
...
int libmpu6000_alloc(mpu6000_ctx_t *ctx, const char *args)
{
	...
	if (err < 0) {
		fprintf(stderr, "mpu6000: Can`t initialize SPI device\n");
		errno = err;
		return -1;
	}
	...
	if (mpu6000_hwSetup(ctx) < 0) {
		fprintf(stderr, "mpu6000: failed to setup device\n");
		return -1;
	}
