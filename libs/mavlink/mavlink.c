...
/* clang-format off */
/* CRC_EXTRA of all payload types in `common.xml` dialect */
...
static const uint8_t crcExtra[] = MAVLINK_MESSAGE_CRCS;
...
static int mav_sendMsg(mav_comp_t *comp, enum mav_msgid msgId, const uint8_t *payload, uint8_t payloadLen)
{
	...
	switch (comp->sys->ver) {
		case mav_version_1:
			...
			for (i = 0; i < payloadLen; i++) {
				buf[MAVLINK_10_LEN_HEADER + i] = payload[i];
			}
	...
	if (write(comp->sys->fd, buf, totalLen) == totalLen) {
		comp->seq++;
		return 0;
	}
