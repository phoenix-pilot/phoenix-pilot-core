...
static void meas_geo2ecef(const meas_geodetic_t *geo, double ecef[3])
{
	...
	ecef[2] = (WGS84_P1MEE * N + geo->h) * geo->sinLat;
}
...
int meas_ecef2geo(const double ecef[3], meas_geodetic_t *geo)
{
	...
	geo->cosLat = cos(geo->lat);
	geo->sinLat = sin(geo->lat);

	geo->cosLon = cos(geo->lon);
	geo->sinLon = sin(geo->lon);

	return 0;
}


void meas_enu2ecef(const vec_t *enu, const meas_geodetic_t *refGeo, const double refEcef[3], double ecef[3])
{
	double rot[9];
	int i;
	...
	/* multiplying transposed rot[3x3] * enu + refEcef */
	for (i = 0; i < 3; i++) {
		ecef[i] = rot[i] * enu->x + rot[3 + i] * enu->y + rot[6 + i] * enu->z + refEcef[i];
	}
}


int meas_en2latlon(vec_t *enu, double *lat, double *lon, double *h)
{
	double ecef[3];
	meas_geodetic_t geo;

	meas_enu2ecef(enu, &meas_common.calib.gps.refGeodetic, meas_common.calib.gps.refEcef, ecef);
	meas_ecef2geo(ecef, &geo);
