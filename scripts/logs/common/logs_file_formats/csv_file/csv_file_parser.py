...
class CsvLogParser:
    ...
    def __parse_imu_log(self, row: list[str]) -> logs_types.ImuLog:
        ...
        gyroDAngle = utils.Vector3(int(row[9]), int(row[10]), int(row[11]))
        mag = utils.Vector3(int(row[12]), int(row[13]), int(row[14]))

        return logs_types.ImuLog(id, timestamp, accel, gyro, gyroDAngle, mag)
    ...
    def __parse_gps_log(self, row: list[str]) -> logs_types.GpsLog:
        ...
        horizontalAcc = int(row[6])
        velocityAcc = int(row[7])
        fix = int(row[8])
        satelliteNumber = int(row[9])
        velocity = utils.NEDCoordinates(int(row[10]), int(row[11]), int(row[12]))

        return logs_types.GpsLog(id, timestamp, position, horizontalAcc, velocityAcc, satelliteNumber, fix, velocity)
