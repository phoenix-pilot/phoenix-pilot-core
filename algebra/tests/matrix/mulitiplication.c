...
TEST(group_matrix_prod_badMats, matrix_prod_badInputMats)
{
	...
	TEST_ASSERT_EQUAL_INT(MAT_PRODUCT_FAIL, matrix_prod(&M1, &M2, &M3));
...
{
	TEST_ASSERT_EQUAL_INT(MAT_PRODUCT_OK, matrix_sparseProd(&M1, &M2, &M3));
...
TEST(group_matrix_sparseProd_badMats, matrix_sparseProd_badInputMats)
{
	...
	/* No matrix is transposed */
	TEST_ASSERT_EQUAL_INT(MAT_PRODUCT_FAIL, matrix_sparseProd(&M1, &M2, &M3));
