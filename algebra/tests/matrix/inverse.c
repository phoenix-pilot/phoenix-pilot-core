...
TEST_SETUP(group_matrix_inv_stdMat)
{
	...
	TEST_ASSERT_EQUAL_INT(BUF_ALLOC_OK, matrix_bufAlloc(&M2, Expected.rows, Expected.cols));
...
{
	TEST_ASSERT_EQUAL_INT(INV_OK, matrix_inv(&M1, &M2, buf, bufLen));
...
{
	TEST_ASSERT_EQUAL_INT(INV_OK, matrix_inv(&M1, &M1, buf, bufLen));
...
TEST_SETUP(group_matrix_inv_bigMat)
{
	...
	/* Allocating result matrix */
	TEST_ASSERT_EQUAL_INT(BUF_ALLOC_OK, matrix_bufAlloc(&M2, M1.rows, M1.cols));

	/* Allocating temporary matrix */
	TEST_ASSERT_EQUAL_INT(BUF_ALLOC_OK, matrix_bufAlloc(&M3, M1.rows, M1.cols));
...
TEST(group_matrix_inv_badMat, matrix_inv_detIsZero)
{
	...
	TEST_ASSERT_EQUAL_INT(INV_FAIL, matrix_inv(&M1, &M2, buf, bufLen));
...
{
	TEST_ASSERT_EQUAL_INT(INV_FAIL, matrix_inv(&M1, &M2, buf, bufLen / 2));
}
